...
export function Checkin() {
  ...
  const getacceptedschedule = async() =>{
    const userid = await AsyncStorage.getItem("userId");
    const response = await api().get(`/api/schedule/awainting-checking/${userid}`);
  ...
  const checking = async(value) => {
    const userid = await AsyncStorage.getItem("userId");
    const response = await api().put(`/api/schedule/checking`, JSON.stringify({scheduleId: value,userId: Number(userid)}));
    if(response.status == 200){
  ...
  return (
    <>
        ...
        <Text style={styles.headerText}>Aguardando Check-in</Text>
      </View>
    <FlatList
...
//     <Badge value={<Text style={{fontSize: 18,color: '#fff'}}>Aceitaram</Text>} status="primary" badgeStyle={{width: 180, height: 30,}}/>
//   </View>
