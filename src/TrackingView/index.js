...
export function TrackingView() {
  ...
  const getcheckedschedule = async() =>{
    const userid = await AsyncStorage.getItem("userId");
    const response = await api().get(`/api/schedule/awainting-checkout/${userid}`);
    if(response.status == 200){
      setChecked(response.data.result);
      console.log(response.data.result);
    }
  }
  const checkout = async(value) => {
    const userid = await AsyncStorage.getItem("userId");
    const response = await api().post(`/api/schedule/checkout`, JSON.stringify({scheduleId: value,userId: Number(userid)}));
    if(response.status == 200){
  ...
  return (
    <>
        ...
        <Text style={styles.headerText}>Viagens Iniciadas</Text>
      </View>
    <FlatList
              ...
              <Card 
                ...
                >
                    <View style={styles.row}>
                      ...
                      <View style={{flex: 0.6, height: 50, flexDirection: 'column'}}>
...
//                 <Badge value={<Text style={{fontSize: 18,color: '#fff'}}>Em progresso</Text>} status="primary" badgeStyle={{width: 180, height: 30,}}/>
//               </View>
...
//                     mode="contained"
//                     onPress={async() => {
//                       await AsyncStorage.setItem("scheduleId",item.scheduleId.toString());
//                       navigation.navigate("TrackingMap");
//                     }}
//                   />
//                 </View>
