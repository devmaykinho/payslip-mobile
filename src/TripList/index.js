...
export function TripList() {
  ...
  const getcheckedschedule = async() =>{
    ...
    if(response.status == 200){
      setCheckout(response.data.result);
  ...
  const starttrip = async(value) => {
    ...
    if(checkin == null){
      ...
      console.log("started trip",value);
    }
  }
  const makeid = (length) => {
    var result           = '';
  ...
  return (
    <>
        ...
        <Text style={styles.headerText}>Relação das Viagens</Text>
      </View>
    <FlatList
...
//                 <Badge value={<Text style={{fontSize: 18,color: '#fff'}}>Viagem Disponível</Text>} status="primary" badgeStyle={{width: 220, height: 30,}}/>
//               </View>
