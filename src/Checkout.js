...
export function Checkout() {
  ...
  const checkout = async(value) => {
    const userid = await AsyncStorage.getItem("userId");
    const response = await api().put(`/api/schedule/checkout`, JSON.stringify({scheduleId: value,userId: Number(userid)}));
    if(response.status == 200){
  ...
  return (
    <>
        ...
        <Text style={styles.headerText}>Aguardando Carregamento</Text>
      </View>
    <FlatList
...
//     <Badge value={<Text style={{fontSize: 18,color: '#fff'}}>Check-in realizado</Text>} status="primary" badgeStyle={{width: 180, height: 30,}}/>
//   </View>
